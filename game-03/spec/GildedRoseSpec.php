...
describe('Gilded Rose', function () {
    ...
    describe('#tick', function () {
        /* agregar lpos productos */

        context ('normal Items', function () {

            /* se actualiza porque se encuentra antes de la fecha de venta */
            it ('updates normal items before sell date', function () {
                $item = GildedRose::of('normal', 10, 5); // quality, sell in X days

                $item->tick();

                /* la calida reduce al iigual que la el tiempo de venta */
                expect($item->quality)->toBe(9);
                expect($item->sellIn)->toBe(4);
            });
            /* se actualiza cuanto está en la fecha de venta */
            it ('updates normal items on the sell date', function () {
                $item = GildedRose::of('normal', 10, 0);

                $item->tick();

                /* la calidad se reduce  */
                expect($item->quality)->toBe(8);
                expect($item->sellIn)->toBe(-1);
            });

            /* después de la fecha de vencimiento, se reduce dos cantidades (calidad)*/
